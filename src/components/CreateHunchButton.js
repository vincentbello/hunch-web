...
export default function CreateHunchButton({ data: { amount, bettee, bettorPickTeamId, gameId, wager }, onCreated }: Props) {
  ...
  useUnsavedAlert(UNSAVED_CHANGES_MESSAGE, !creating);

  return (
    <>
      <Prompt when={!creating} message={nextLocation => nextLocation.pathname.startsWith('/hunch/new') ? true : UNSAVED_CHANGES_MESSAGE} />
